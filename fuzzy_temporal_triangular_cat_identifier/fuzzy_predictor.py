...
class FuzzyPredictor:
    ...
    def confidence_algorithm_metadata(self) -> Dict[str, Any]:
        return {
            ...
            "description": "Confidence is the max normalized combined score after weight membership, triangular-selected dynamic sigma, optional overlap/consistency penalties, and temporal prior.",
            "parameters": {
    ...
    def compute_weight_membership(self, row, catalog, sigma_map=None, overlap_map=None, consistency_map=None):
        ...
        for enroll_id, ref_weight_kg in catalog.items():
            sigma_kg = sigma_map.get(enroll_id, self.config.base_sigma_kg) if self.config.use_dynamic_sigma_in_prediction and sigma_map is not None else self.config.base_sigma_kg
            raw_scores[enroll_id] = self.diagnostic.fuzzy_membership_with_sigma(measured_weight_kg, ref_weight_kg, sigma_kg)
    ...
    def predict_for_day(self, df_day, catalog, sigma_map=None, overlap_map=None, consistency_map=None, device_col="_device"):
        if df_day.empty:
            return pd.DataFrame(), {}
        ...
        for _, row in df_day.iterrows():
            ...
            temporal_prior = {cat: decay * latent_state.get(cat, 0.0) + (1.0 - decay) * uniform for cat in cats}
            temporal_prior = self.normalize_scores(temporal_prior)
            combined = {cat: (1.0 - self.config.temporal_weight) * current_membership.get(cat, 0.0) + self.config.temporal_weight * temporal_prior.get(cat, 0.0) for cat in cats}
            ...
            for cat in cats:
                ...
                if sigma_map is not None:
                    ...
                    row_result[f"dynamic_sigma_lb_{cat}"] = round(sigma_map.get(cat, self.config.base_sigma_kg) * self.config.kg_to_lb, 6)
    ...
    def run_user_prediction(self, df_user, catalog, inconsistency_df):
        sigma_map, consistency_map, overlap_map = self.diagnostic.build_prediction_artifacts_from_inconsistency(inconsistency_df, catalog)
        ...
        all_day_rows, all_day_summaries = [], []
        for _, df_day in df_user.groupby(["event_date", dev_col]):
            row_df, day_summary = self.predict_for_day(df_day, catalog, sigma_map, overlap_map, consistency_map, dev_col)
            if not row_df.empty:
                all_day_rows.append(row_df)
            if day_summary:
                all_day_summaries.append(day_summary)
        return pd.concat(all_day_rows, ignore_index=True) if all_day_rows else pd.DataFrame(), pd.DataFrame(all_day_summaries)